...
_REGION_RE = re.compile(r"^region-[a-z0-9]+(-[a-z0-9]+)*\Z")
...
def reject_dummy_project(project_id: str):
    ...
    Note: 'example-sandbox' is a placeholder from imported dummy snapshots that 
    ...
    if cleaned in ("your-project-id", "example-sandbox", "dummy-project-id", "dummy-project"):
        raise HTTPException(
            status_code=400,
...
def run_query_and_log(client: bigquery.Client, sql: str, label: str = "Query",
    ...
    loc = bare_region(req_region) if req_region and isinstance(req_region, str) else None
    t0 = time.time()
