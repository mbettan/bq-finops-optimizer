...
REPORT_MODULES: List[Dict[str, Any]] = [
    ...
    {"keys": ["bq_time_travel_results"],       "endpoint": "/api/storage/time_travel_reduction","label": "Time Travel Reduction",            "section": 5, "billable_note": None},
    {"keys": ["bq_shard_results"],             "endpoint": "/api/storage/shard_consolidation",  "label": "Shard Consolidation",             "section": 5, "billable_note": None},
...
# Extra keys not in REPORT_MODULES but present in snapshots and used by findings
REPORT_RELEVANT_KEYS.add("bq_resource_warning_results")
...
# ── Absolute dollar impact points (prevents low-score demotion) ───
# Ensures findings with substantial hard-dollar savings are never
# demoted to Info just because they're a small % of a large org baseline.
_ABS_DOLLAR_THRESHOLDS = [
    (5000, 13),   # $60k/yr → Critical driver
    (1000,  8),   # $12k/yr → High driver
    (250,   5),   # $3k/yr  → Medium driver
    (50,    3),   # $600/yr → Low driver
    (0,     2),   # Any positive savings
]
...
def _abs_impact_points(impact_usd: float) -> int:
    """Map absolute monthly dollar savings to impact points."""
    for threshold, pts in _ABS_DOLLAR_THRESHOLDS:
        if impact_usd >= threshold:
            return pts
    return 1
...
_FINDING_META: dict[str, dict[str, str]] = {
    ...
    "TIME_TRAVEL_EXCESS":{"pillar": "Storage",    "lever": "Rate",  "horizon": "M1",      "risk": "0", "confidence": "High"},
    "SHARD_CONSOLIDATION":{"pillar": "Storage",  "lever": "Usage", "horizon": "Q1",      "risk": "0", "confidence": "High"},
}
...
_FINDING_DEFS: list[dict[str, Any]] = [
    {
        ...
        "keys": ["bq_top_spenders", "bq_job_results"],
        "keys_any": True,  # Trigger if ANY key is present (not all)
    },
    {
    ...
    {
        "finding_id": "TIME_TRAVEL_EXCESS", "ref_id": "ID-19",
        "title": "Time-Travel Window Reduction Opportunity",
        "priority": "Medium", "effort": "Low", "category": "Storage & Lifecycle",
        "description": "Datasets with the default 7-day time-travel window can often be safely reduced to 2 days, reclaiming significant physical storage on high-churn tables.",
        "recommendation": "Reduce max_time_travel_hours to 48h on eligible datasets via ALTER SCHEMA DDL. Review each dataset's recovery requirements before applying.",
        "official_docs_url": "https://cloud.google.com/bigquery/docs/time-travel",
        "source_module": "time_travel",
        "keys": ["bq_time_travel_results"],
    },
    {
        "finding_id": "SHARD_CONSOLIDATION", "ref_id": "ID-20",
        "title": "Date-Sharded Tables \u2192 Partitioned Tables",
        "priority": "Medium", "effort": "Medium", "category": "Storage & Lifecycle",
        "description": "Date-sharded table families (e.g. events_20240101, events_20240102, \u2026) bypass partition pruning, require wildcard queries, and inflate metadata overhead compared to a single ingestion-time partitioned table.",
        "recommendation": "Consolidate each sharded family into a single partitioned table using bq cp --append or a scheduled MERGE pipeline. Retain the shards briefly for rollback, then delete.",
        "official_docs_url": "https://cloud.google.com/bigquery/docs/partitioned-tables",
        "source_module": "shard_consolidation",
        "keys": ["bq_shard_results"],
    },
]
...
class FindingsSynthesizer:
    ...
    def __init__(self, data: dict[str, Any]) -> None:
        self._data = data
        region = data.get("bq_region", "region-us")
        self._pricing = get_pricing(region)
    ...
    def _check_trigger(self, fdef: dict[str, Any]) -> tuple[bool, Optional[float], dict[str, str]]:
        ...
        # Precondition: source keys must be present and non-empty.
        # If keys_any=True, trigger if ANY key is present (not all).
        if fdef.get("keys_any"):
            if not any(self._key_present_and_nonempty(k) for k in fdef["keys"]):
        ...
        if fid == "PRICING_MODEL":
            ...
            if isinstance(top, list) and top:
                ...
                window_ondemand_cost = (total_bytes / (1024**4)) * self._pricing.on_demand_usd_per_tib
            ...
            if editions_cost > 0 and editions_cost < ondemand_cost * 0.9:
                ...
                enrichment["evidence"] = (
                    f"Current On-Demand spend: {_format_usd(ondemand_cost)}/mo vs "
                    f"Editions 3yr projection: {_format_usd(editions_cost)}/mo "
                    f"(measured avg: {avg_slots:,.1f} slots; Enterprise break-even: "
                    f"{breakeven_enterprise:,.0f} slots)."
                )
                ...
                enrichment["evidence"] = (
                    f"Current On-Demand spend: {_format_usd(ondemand_cost)}/mo vs "
                    f"Editions projection: {_format_usd(editions_cost)}/mo "
                    f"(measured avg: {avg_slots:,.1f} slots; Enterprise break-even: "
                    f"{breakeven_enterprise:,.0f} slots)."
                )
        ...
        if fid == "MICRO_QUERY":
            ...
            if isinstance(top, list):
                ...
                if high_freq:
                    ...
                    floor_cost = floor_tb * self._pricing.on_demand_usd_per_tib
        ...
        if fid == "SELECT_STAR":
            ...
            if isinstance(linter, list):
                _SELECT_STAR_TOKENS = ("SELECT *", "SELECT_STAR", "SELECTSTAR")
                select_star = [
                    r for r in linter
                    if isinstance(r, dict) and any(
                        tok in (r.get("abuse_type") or "").upper()
                        or tok in (r.get("pattern") or "").upper()
                        for tok in _SELECT_STAR_TOKENS
                    )
                ]
                if select_star:
                    ...
                    impact = (total_gb / 1024) * self._pricing.on_demand_usd_per_tib  # Convert GB to TiB
        ...
        if fid == "BATCH_ELIGIBLE":
            ...
            if isinstance(batch, list) and batch:
                ...
                total_cost = total_slot_hrs * self._pricing.editions_slot_hr_rate
        ...
        if fid == "STALE_TABLES":
            ...
            if isinstance(hygiene, list) and hygiene:
                # Dedup by table_name to prevent fan-out inflation
                hygiene = self._dedup_rows(hygiene, ["project_id", "dataset", "table_name"], "live_active_physical_gb")
                stale = [
                    r for r in hygiene
                    if isinstance(r, dict)
                    and str(r.get("health_status", "")).lower() not in ("healthy", "")
                ]
                if stale:
                    total_gb = sum(sf(r.get("live_active_physical_gb")) for r in stale)
                    total_savings = sum(sf(r.get("monthly_savings")) for r in stale)
                    ...
                    if total_savings > 0:
                        enrichment["evidence"] += f" Reducible TT savings: {_format_usd(total_savings)}/mo."
        ...
        if fid == "GUARDRAILS":
            ...
            if isinstance(gov, dict):
                ...
                if has_quota_keys and not any(r.get("custom_quota") or r.get("has_custom_quota") for r in gov if isinstance(r, dict)):
                    ...
                    return True, None, enrichment
            # Check actual /api/governance/analyze response shape
            if isinstance(gov, dict):
                exp_issues = gov.get("expiration_issues") or []
                flt_issues = gov.get("filter_issues") or []
                if isinstance(exp_issues, list) and isinstance(flt_issues, list):
                    if exp_issues or flt_issues:
                        parts = []
                        if flt_issues:
                            parts.append(f"{len(flt_issues)} partitioned table(s) missing require_partition_filter")
                        if exp_issues:
                            parts.append(f"{len(exp_issues)} dataset(s) missing default table expiration")
                        enrichment["evidence"] = "; ".join(parts) + "."
                        return True, None, enrichment
            return False, None, enrichment
        ...
        if fid == "MANUAL_BACKUPS":
            ...
            if isinstance(static, list):
                # Dedup by table_id
                static = self._dedup_rows(static, ["project_id", "dataset_id", "table_id"], "size_bytes")
                ...
                if backups:
                    ...
                    # Storage cost: regional active rate per GiB/mo
                    impact = total_bytes / (1024**3) * self._pricing.active_logical_gib_mo
                    return True, round(impact, 2) if impact > 1 else None, enrichment
            return False, None, enrichment
        ...
        if fid == "LTS_FORFEITURE":
            ...
            if isinstance(hygiene, list):
                ...
                if high_churn:
                    ...
                    enrichment["evidence"] = f"{len(high_churn)} tables with churn >0.5 totalling {total_gb:,.0f} GB. LTS penalty: ~{_format_usd(total_gb * self._pricing.long_term_logical_gib_mo)}/mo. Top: {'; '.join(parts)}"
                    return True, round(total_gb * self._pricing.long_term_logical_gib_mo, 2), enrichment
            return False, None, enrichment
        ...
        # ── ID-19: Time Travel Excess ─────────────────────────────────
        if fid == "TIME_TRAVEL_EXCESS":
            tt = self._data.get("bq_time_travel_results")
            if isinstance(tt, list) and tt:
                total_savings = sum(sf(r.get("monthly_savings")) for r in tt if isinstance(r, dict))
                if total_savings > 1:
                    top3 = sorted(
                        [r for r in tt if isinstance(r, dict) and sf(r.get("monthly_savings")) > 0],
                        key=lambda r: -sf(r.get("monthly_savings")),
                    )[:3]
                    parts = [f"{r.get('dataset_id', '?')} ({_format_usd(sf(r.get('monthly_savings')))})"
                             for r in top3]
                    enrichment["evidence"] = (
                        f"{len(tt)} datasets with reducible time-travel windows, "
                        f"total savings {_format_usd(total_savings)}/mo. Top: {'; '.join(parts)}"
                    )
                    return True, round(total_savings, 2), enrichment
            return False, None, enrichment

        # ── ID-20: Shard Consolidation ────────────────────────────────
        if fid == "SHARD_CONSOLIDATION":
            shards = self._data.get("bq_shard_results")
            if isinstance(shards, list) and shards:
                total_families = len(shards)
                total_shard_count = sum(sf(r.get("shard_count")) for r in shards if isinstance(r, dict))
                total_gib = sum(sf(r.get("total_physical_gib")) for r in shards if isinstance(r, dict))
                top3 = sorted(
                    [r for r in shards if isinstance(r, dict)],
                    key=lambda r: -sf(r.get("shard_count")),
                )[:3]
                parts = [f"{r.get('table_prefix', '?')}_* ({int(sf(r.get('shard_count')))} shards)" for r in top3]
                enrichment["evidence"] = (
                    f"{total_families} date-sharded table families with {int(total_shard_count):,} total shards "
                    f"({total_gib:,.1f} GiB). Top: {'; '.join(parts)}"
                )
                return True, None, enrichment
            return False, None, enrichment

        if fid == "CAPACITY_CEILING":
            tiered_raw = self._data.get("bq_slots_tiered")
            rows = (
                tiered_raw
                if isinstance(tiered_raw, list)
                else ([tiered_raw] if isinstance(tiered_raw, dict) else [])
            )
            risky_res: list[tuple[str, float, float, float]] = []
            total_spike_slot_hrs = 0.0
            for r in rows:
                if not isinstance(r, dict):
                    continue
                res_id = r.get("reservation_id", "default")
                p95 = sf(r.get("balanced_baseline_p95"))
                perf_max = sf(r.get("performance_baseline_max"))
                auto_max = sf(r.get("suggested_autoscale_max")) or (perf_max * 1.2)
                mins = sf(r.get("minutes_observed")) or 60.0
                if p95 > 0 and auto_max > p95 * 1.5:
                    spike_slots = auto_max - p95
                    # Estimate monthly autoscale exposure assuming spikes occur ~5% of observed window
                    est_spike_hrs = spike_slots * (mins / 60.0) * 0.05
                    total_spike_slot_hrs += est_spike_hrs
                    risky_res.append((res_id, p95, auto_max, spike_slots))
            if risky_res:
                parts = [
                    f"{res_id} (P95: {p95:,.0f} slots → Peak: {auto_max:,.0f}"
                    f" slots, {auto_max / p95:.1f}× spike)"
                    for res_id, p95, auto_max, _ in risky_res[:3]
                ]
                est_risk_usd = round(
                    total_spike_slot_hrs * self._pricing.editions_slot_hr_rate, 2
                )
                enrichment["evidence"] = (
                    f"{len(risky_res)} reservation(s) exhibit peak slot demand >1.5×"
                    f" P95 baseline: {'; '.join(parts)}."
                )
                enrichment["affected_objects"] = ", ".join(r[0] for r in risky_res[:5])
                return True, est_risk_usd if est_risk_usd > 10 else None, enrichment
            return False, None, enrichment
        ...
        if fid == "UNPARTITIONED":
            ...
            if isinstance(static, list) and static:
                # Dedup by table_id to prevent fan-out inflation
                static = self._dedup_rows(static, ["project_id", "dataset_id", "table_id"], "size_bytes")
                unpartitioned = [
                    r for r in static
                    if isinstance(r, dict)
                ...
                if unpartitioned:
                    ...
                    # Estimate conservative scan-reduction savings:
                    # Partitioning + require_partition_filter eliminates ~50% of
                    # full-table scans for tables > 10 GiB.
                    est_savings = total_tb * self._pricing.on_demand_usd_per_tib * 0.5
                    # Cap at 15% of compute baseline to avoid inflated estimates
                    top = self._data.get("bq_top_spenders")
                    if isinstance(top, list) and top:
                        lookback = self._data.get("_lookback_days", 30) or 30
                        monthly_factor = 30.0 / max(lookback, 1)
                        ...
                        ) / (1024**4) * self._pricing.on_demand_usd_per_tib * monthly_factor
                        if local_baseline > 0:
                            est_savings = min(est_savings, local_baseline * 0.15)
                    impact_val = round(est_savings, 2) if est_savings > 10 else None
                    return True, impact_val, enrichment
        ...
        # ── ID-06: DML Abuse ─────────────────────────────────────────
        if fid == "DML_ABUSE":
            dml = self._data.get("bq_antipatterns_results")
            if isinstance(dml, list) and dml:
                dml_rows = [r for r in dml if isinstance(r, dict)]
                total_slot_hrs = sum(sf(r.get("wasted_slot_hours")) for r in dml_rows)
                total_impact = round(total_slot_hrs * self._pricing.editions_slot_hr_rate, 2)
                top3 = sorted(dml_rows, key=lambda r: -sf(r.get("wasted_slot_hours")))[:3]
                parts = [
                    f"{r.get('dest_table_id') or r.get('user_email', '?')} ({sf(r.get('wasted_slot_hours')):.1f} slot-hrs)"
                    for r in top3
                ]
                enrichment["evidence"] = (
                    f"{len(dml_rows)} DML abuse patterns, {total_slot_hrs:,.1f} wasted slot-hours. "
                    f"Top: {'; '.join(parts)}"
                )
                return True, total_impact if total_impact > 0 else None, enrichment
            return False, None, enrichment

        # ── ID-08: MV Candidates ─────────────────────────────────────
        if fid == "MV_CANDIDATES":
            mv = self._data.get("bq_mv_results")
            if isinstance(mv, list) and mv:
                mv_rows = [r for r in mv if isinstance(r, dict)]
                total_slot_hrs = sum(sf(r.get("total_slot_hours")) for r in mv_rows)
                total_impact = round(total_slot_hrs * self._pricing.editions_slot_hr_rate, 2)
                top3 = sorted(mv_rows, key=lambda r: -sf(r.get("total_slot_hours")))[:3]
                parts = [
                    f"{r.get('table_name', '?')} ({sf(r.get('total_slot_hours')):.1f} slot-hrs, {sf(r.get('refresh_count')):.0f} refreshes)"
                    for r in top3
                ]
                enrichment["evidence"] = (
                    f"{len(mv_rows)} MV candidates, {total_slot_hrs:,.1f} total slot-hours. "
                    f"Top: {'; '.join(parts)}"
                )
                return True, total_impact if total_impact > 0 else None, enrichment
            return False, None, enrichment
        ...
        # ── ID-13: HBO ────────────────────────────────────────────────
        if fid == "HBO":
            hbo = self._data.get("bq_hbo_results")
            if isinstance(hbo, list) and hbo:
                hbo_rows = [r for r in hbo if isinstance(r, dict)]
                total_savings = sum(sf(r.get("estimated_savings_usd")) for r in hbo_rows)
                total_slot_hrs = sum(sf(r.get("saved_slot_hours")) for r in hbo_rows)
                top3 = sorted(hbo_rows, key=lambda r: -sf(r.get("estimated_savings_usd")))[:3]
                parts = [
                    f"job {r.get('job_id', '?')[:12]}\u2026 ({_format_usd(sf(r.get('estimated_savings_usd')))})"
                    for r in top3
                ]
                enrichment["evidence"] = (
                    f"{len(hbo_rows)} HBO-optimized jobs, {total_slot_hrs:,.1f} saved slot-hours "
                    f"({_format_usd(total_savings)}/mo). Top: {'; '.join(parts)}"
                )
                return True, round(total_savings, 2) if total_savings > 0 else None, enrichment
            return False, None, enrichment

        # ── ID-07: Slot Skew / Hot Spots ─────────────────────────────
        if fid == "SLOT_SKEW":
            skew = self._data.get("bq_skew_results")
            if isinstance(skew, list) and skew:
                skew_rows = [r for r in skew if isinstance(r, dict)]
                high_skew = sorted(
                    skew_rows,
                    key=lambda r: -sf(r.get("skew_ratio") or r.get("max_compute_ms") or 0),
                )
                top3 = high_skew[:3]
                parts = [
                    f"job {str(r.get('job_id', '?'))[:12]}\u2026 stage"
                    f" {r.get('stage_name', '?')} (skew"
                    f" {sf(r.get('skew_ratio')):.1f}\u00d7, max"
                    f" {sf(r.get('max_compute_ms')) / 1000:.1f}s)"
                    for r in top3
                ]
                enrichment["evidence"] = (
                    f"{len(skew_rows)} skewed execution stages detected. "
                    f"Top hotspots: {'; '.join(parts)}."
                )
                enrichment["affected_objects"] = ", ".join(
                    sorted({str(r.get('user_email', '?')) for r in top3})
                )
                return True, None, enrichment
            return False, None, enrichment

        # ── ID-14: Performance Resource Warnings ─────────────────────
        if fid == "PERF_WARNINGS":
            perf = self._data.get("bq_performance_results")
            rw = self._data.get("bq_resource_warning_results")
            warning_count = 0
            parts: list[str] = []
            affected: set[str] = set()
            if isinstance(perf, dict):
                sc = perf.get("slot_contention_jobs") or []
                sq = perf.get("shuffle_quota_jobs") or []
                dv = perf.get("data_volume_jobs") or []
                warning_count += len(sc) + len(sq) + len(dv)
                if sc:
                    parts.append(f"{len(sc)} slot contention jobs")
                if sq:
                    parts.append(f"{len(sq)} shuffle quota exhaustion jobs")
                if dv:
                    parts.append(f"{len(dv)} large data volume jobs")
            elif isinstance(perf, list) and perf:
                warning_count += len(perf)
                parts.append(f"{len(perf)} performance warning events")
            if isinstance(rw, list) and rw:
                rw_dicts = [r for r in rw if isinstance(r, dict)]
                warning_count += len(rw_dicts)
                for r in rw_dicts[:5]:
                    if r.get("user_email"):
                        affected.add(r["user_email"])
                top_msg = (
                    str(rw_dicts[0].get("resource_warning", ""))[:80] if rw_dicts else ""
                )
                if top_msg:
                    parts.append(
                        f"{len(rw_dicts)} resource warning events"
                        f' (e.g. "{top_msg}\u2026")'
                    )
            if warning_count > 0:
                enrichment["evidence"] = (
                    f"{warning_count} performance/resource warning signals"
                    f" identified: {'; '.join(parts)}."
                )
                if affected:
                    enrichment["affected_objects"] = ", ".join(sorted(affected)[:5])
                return True, None, enrichment
            return False, None, enrichment
    ...
    def synthesize(self) -> list[Finding]:
        ...
        if isinstance(top, list):
            window_bytes = sum(
                sf(r.get("total_bytes_billed")) for r in top if isinstance(r, dict)
            ) / (1024**4) * self._pricing.on_demand_usd_per_tib
        ...
        for fdef in all_defs:
            ...
            if triggered:
                ...
                if impact and impact > 0:
                    ...
                    # Use the HIGHER of relative-% and absolute-$ impact points
                    # so findings with real dollar savings aren't penalized by
                    # a large org baseline.
                    rel_pts = _impact_points(savings_pct)
                    abs_pts = _abs_impact_points(impact)
                    imp_pts = max(rel_pts, abs_pts)
                    impact_pct = savings_pct * 100
                else:
                    # Derive imp_pts from intended priority so structural findings
                    # (no dollar impact) aren't all collapsed to "Info".
                    target_priority = enrichment.get("priority", fdef["priority"])
                    imp_pts = {"Critical": 13, "High": 10, "Medium": 5, "Low": 2}.get(target_priority, 1)
                    impact_pct = None
...
def _classify_storage_risk(row: dict) -> str:
    """Classify a Static Audit row into a 4-tier risk scale.

    Mirrors the frontend ``classifyStorageRisk()`` in app.js.
    Thresholds: >1 TiB or >1B rows = "large"; <1 GiB = negligible.
    """
    size_gib = (row.get("size_bytes") or 0) / (1024 ** 3)
    rows = row.get("row_count") or 0
    managed = (1 if row.get("is_partitioned") else 0) + (1 if row.get("is_clustered") else 0)
    is_large = size_gib > 1024 or rows > 1e9
    if size_gib < 1:
        return "Low Risk"
    if is_large and managed == 0:
...
_RISK_BADGE_COLORS: dict[str, str] = {
    "Critical Risk": "background:rgba(239,68,68,0.15);color:#ef4444",
    "High Risk":     "background:rgba(251,146,60,0.15);color:#fb923c",
    "Medium Risk":   "background:rgba(250,204,21,0.15);color:#ca8a04",
    "Low Risk":      "background:rgba(74,222,128,0.15);color:#22c55e",
}


def _format_risk_badge(val: Any, **_kw: Any) -> str:
    """Render a risk tier as a styled inline badge."""
    label = str(val) if val else "\u2014"
    style = _RISK_BADGE_COLORS.get(label, "")
    if style:
        return f'<span style="{style};font-weight:600;padding:2px 8px;border-radius:4px;font-size:0.85em">{_esc(label)}</span>'
    return _esc(label)
...
class HTMLReportRenderer:
    ...
    def _compute_baseline(self) -> dict[str, float]:
        ...
        if isinstance(top, list):
            ...
            b["window_on_demand_cost"] = b["bytes_billed_tib"] * self._pricing.on_demand_usd_per_tib
        ...
        # If avg_slots is near zero, try the slots API for better data
        ...
        # Storage baseline — include all storage categories
        storage = self._data.get("bq_storage_results")
        if isinstance(storage, dict):
        ...
        if isinstance(hygiene, list):
            ...
            for r in hygiene:
                if isinstance(r, dict):
                    ...
                    tt_key = f"{r.get('project_id', '')}|{r.get('dataset', '')}|{r.get('table_name', '')}"
                    if tt_gb > 0 and tt_key not in seen_tt:
                        seen_tt.add(tt_key)
                        b["storage_cost"] += tt_gb * self._pricing.active_physical_gib_mo
  ...
  <ol class="toc-list">{items}</ol>
    ...
    def _section_1_executive_summary(self) -> str:
        ...
        if b["bytes_billed_tib"] > 0:
            derived_cost = b["bytes_billed_tib"] * self._pricing.on_demand_usd_per_tib
            if derived_cost > 0:
                jobs_cost = 0.0
                jobs = self._data.get("bq_job_results")
                if isinstance(jobs, dict):
...
<table class="data-table baseline-table">
  ...
  <tbody>
    ...
    <tr><td><strong>On-demand compute cost</strong></td><td class="num"><strong>{_format_usd(b["on_demand_cost"])}/mo</strong></td><td>TiB × ${self._pricing.on_demand_usd_per_tib}/TiB{reconciliation_pct}</td></tr>
    <tr><td>Average slots</td><td class="num">{b["avg_slots"]:,.0f}</td><td><code>SUM(total_slot_ms)/3.6e6 ÷ 730h</code></td></tr>
    <tr><td>Editions cost (3yr rate)</td><td class="num">{_format_usd(editions_cost)}/mo</td><td>Slot simulation (3yr commit baseline + PAYG autoscale)</td></tr>
    ...
    <tr><td><strong>Total in-scope spend</strong></td><td class="num"><strong>{_format_usd(b["total_in_scope"])}/mo</strong></td><td>Compute + storage</td></tr>
        ...
        content += self._callout("reconciliation",
            f'On-demand cost derives from <code>SUM(total_bytes_billed)</code> × ${self._pricing.on_demand_usd_per_tib}/TiB. '
...
<div class="dimension-card">
  ...
  <div class="dimension-body">
    ...
    <p><strong>Focus Areas:</strong> Comparing On-Demand (${self._pricing.on_demand_usd_per_tib}/TiB) against Editions slot-hour pricing;
...
<table class="data-table">
  ...
  <tbody>
    <tr><td>On-Demand</td><td>{_format_usd(ondemand_cost)}</td><td>${self._pricing.on_demand_usd_per_tib}/TiB</td></tr>
    <tr><td>Editions (Projected)</td><td>{_format_usd(editions_cost)}</td><td>${self._pricing.editions_slot_hr_rate}/slot-hr</td></tr>
  </tbody>
</table>"""
...
<p class="verdict">{verdict}</p>
    ...
    def _derive_billing_model(self, cost: float, bytes_billed: float) -> str:
        ...
        if bytes_billed > 0:
            ...
            expected = tib * self._pricing.on_demand_usd_per_tib
    ...
    def _section_4_query(self) -> str:
        ...
        if top and isinstance(top, list):
            ...
            content += f'<div class="table-footnote"><em>Billing Model is inferred: if cost reconciles to bytes\u00d7${self._pricing.on_demand_usd_per_tib}/TiB \u00b115%, it is On-Demand. Service accounts with zero bytes billed but real cost are slot-attributed (Reservation).</em></div>'

        linter = self._data.get("bq_linter_results")
        if linter:
            # Unescape any pre-encoded HTML entities in query snippets
    ...
    def _filter_storage(datasets: list[dict]) -> list[dict]:
        ...
        seen: dict[str, dict] = {}
        for d in filtered:
            key = f"{d.get('project_name', '')}|{d.get('dataset_name', '')}"
            if key in seen:
    ...
    def _section_5_storage(self) -> str:
        ...
        if static and isinstance(static, list):
            ...
            # Inject computed risk tier (mirrors app.js classifyStorageRisk)
            for row in static:
                if isinstance(row, dict) and "risk_tier" not in row:
                    row["risk_tier"] = _classify_storage_risk(row)
            # Deduplicate by (dataset_id, table_id)
            seen_static: dict[str, dict] = {}
            for row in static:
                if not isinstance(row, dict):
                    continue
                key = f"{row.get('project_id', '')}|{row.get('dataset_id', '')}|{row.get('table_id', '')}"
                if key not in seen_static:
                    seen_static[key] = row
            static_deduped = list(seen_static.values())
            content += self._render_table(
                "Static Audit", static_deduped,
                ["table_id", "dataset_id", "risk_tier", "is_partitioned", "is_clustered", "size_bytes", "issue"],
        ...
        hygiene = self._data.get("bq_hygiene_results")
        if hygiene:
            # Deduplicate by (dataset, table_name)
            seen_hyg: dict[str, dict] = {}
            for row in hygiene:
                if not isinstance(row, dict):
                    continue
                key = f"{row.get('project_id', '')}|{row.get('dataset', '')}|{row.get('table_name', '')}"
        ...
        tt_data = self._data.get("bq_time_travel_results")
        if tt_data:
            content += self._render_table(
                "Time Travel Reduction", tt_data,
                ["dataset_id", "billing_model", "current_time_travel_days", "time_travel_gib", "monthly_savings", "ddl"],
                ["Dataset", "Billing", "Current Window (d)", "TT (GiB)", "Savings ($/mo)", "DDL"],
                formatters={
                    "dataset_id": lambda v, row=None: _bq_link(v, kind="dataset",
                        project=row.get("project_id", "") if row else ""),
                    "time_travel_gib": _format_round2,
                    "monthly_savings": _format_usd,
                },
            )

        shard_data = self._data.get("bq_shard_results")
        if shard_data:
            content += self._render_table(
                "Shard Consolidation", shard_data,
                ["table_prefix", "dataset_id", "shard_count", "total_physical_gib", "risk_level"],
                ["Table Prefix", "Dataset", "Shards", "Physical (GiB)", "Risk"],
                formatters={
                    "dataset_id": lambda v, row=None: _bq_link(v, kind="dataset",
                        project=row.get("project_id", "") if row else ""),
                    "total_physical_gib": _format_round2,
                },
            )
...
<table class="data-table">
  ...
  <tbody>{passed_items}</tbody>
        ...
        # Sort within each phase so dependencies come before dependents
...
<table class="data-table">
  ...
  <tbody>
    ...
    <tr><td>On-Demand</td><td>Pay-per-query at <strong>${self._pricing.on_demand_usd_per_tib}/TiB</strong> billed. No commitment; 2,000 concurrent slot cap per project.</td></tr>
