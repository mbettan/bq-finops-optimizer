...
"""
===============================================================================
Enterprise High-Performance Concurrent BigQuery Recommendations & Insights Scanner
===============================================================================
...
from .constants import bare_region, normalize_region
...
def discover_projects_via_table_storage(billing_project, region="us"):
    ...
    Accepts either region convention — "us" or "region-us". Callers
    upstream (the UI, every Pydantic model) use the prefixed form; this function
    historically required the bare form and would silently build an invalid
    ``region-region-us`` qualifier when handed a prefixed value.
    """
    qualifier = normalize_region(region)   # 'region-us'
    location = bare_region(region)         # 'us'

    sql = (
        f"SELECT DISTINCT project_id "
        f"FROM `{qualifier}`.INFORMATION_SCHEMA.TABLE_STORAGE_BY_ORGANIZATION "
    ...
    if client is not None:
        try:
            ...
            print(f"[WARNING] BQ SDK query failed for region {location}: {e}", file=sys.stderr)
            # Fall through to CLI
    ...
    try:
        ...
        print(
            f"[WARNING] Failed to query TABLE_STORAGE_BY_ORGANIZATION for region {location}: {e}",
            file=sys.stderr,
        )
        return []
