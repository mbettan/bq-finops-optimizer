# Release Notes — FinOps Optimizer for BigQuery

This page documents production updates to FinOps Optimizer for BigQuery. We recommend that users periodically check this list for any new announcements.
...
**Breaking**
* Removed the `AUTH_ENFORCED_UPSTREAM` environment variable. The application no longer requires any environment variable to start. Existing deployments that set it are unaffected — the variable is simply ignored.

**Feature (Native Windows Support)**
* Added native Windows launchers: `run.bat` (Command Prompt) and `run.ps1` (PowerShell). Double-click `run.bat` to auto-detect Python, create a virtual environment, install dependencies (cached via SHA-256 sentinel), and open the browser when the server is ready. Handles Windows Store Python stubs and missing `venv` module gracefully with `virtualenv` fallback.
* Added `run.sh` launcher for macOS/Linux parity with the same readiness-poll, cached-dependency, and security-notice UX.
* All launchers hard-pin `--host 127.0.0.1` and print a per-launch security notice reminding operators not to expose the unauthenticated app on shared hosts.

**Documentation**
* Added README "Option 2: Local Development (Windows)" with one-click launch instructions, manual PowerShell setup, environment variable guidance, and a troubleshooting table covering Windows Store stubs, missing `venv`, file locking, and `python3` alias differences.
* Updated README testing section with cross-platform commands (macOS/Linux and Windows).
* Cloud Shell section updated from "Ideal for Windows Users" to "Zero Setup Path" — now that native Windows is supported, Cloud Shell is recommended for enterprise-restricted PCs rather than all Windows users.

**Cross-Platform Hardening**
* Converted `os.path` usages in `src/main.py` to `pathlib.Path` for portable path handling across Windows, macOS, and Linux.
* Added explicit `encoding="utf-8"` to all bare `open()` calls in `src/main.py` and `src/cost_attribution.py`, preventing CP-1252 encoding issues on Windows while remaining a no-op on UTF-8 Linux/macOS systems.

**Dependencies**
* Added `httpx2>=2.0.0` to `requirements-dev.txt`. Starlette 1.4+ imports `httpx2` (now maintained under the Pydantic org) as the preferred HTTP client for `TestClient`, eliminating deprecation warnings. Also added `ruff`, `playwright`, and `pytest-playwright` as required dev dependencies for linting and UI smoke testing.

**Security**
* Replaced the unconditional startup refusal with a targeted startup warning that fires only when the server is bound to a non-loopback address outside a managed platform. Loopback runs (local development, Cloud Shell Web Preview) are silent; Cloud Run is exempt because IAM is the boundary there. The service still has no built-in authentication and must run behind Cloud Run IAM or IAP when exposed.
* Introduced enterprise least-privilege IAM specification (`deploy/roles/bq_finops_reader.yaml`) that formally isolates control-plane metadata telemetry from the data plane. The role deliberately excludes `bigquery.tables.getData`, guaranteeing that the application cannot read customer table contents.
* Added machine-verifiable permission auditing script (`deploy/check_permissions.py`) to assert both positive telemetry permissions and the absence of data-plane permissions (`bigquery.tables.getData`).

**Feature (Storage Risk Status Semantics)**
* Reclassified the Storage Optimizer risk model from an aggressive 2-state binary into a 4-tier risk scale: **Critical Risk**, **High Risk**, **Medium Risk**, and **Low Risk**.
* Mid-sized tables under 1 TiB that use either partitioning or clustering are now graded **Low Risk**. Classification accounts for table scale (>1 TiB or >1B rows) alongside partition and clustering hygiene (`is_partitioned`, `is_clustered`).
* Replaced inline styles with theme-aware CSS classes and added explanatory tooltips detailing the specific threshold that triggered each rating.

**Feature (Corporate Light Mode & Dual Theme Engine)**
* Added a Google Cloud Console-styled Corporate Light Mode alongside the signature Dark Theme, with strict WCAG AA contrast compliance (≥ 4.5:1 text-to-background ratio) to ensure charts and tables are presentation-ready for CFO decks and executive reviews.
* Engineered a two-layer CSS token architecture (`Layer 1` palette triplets + `Layer 2` application mappings) with a pre-paint bootstrap script (`theme-boot.js`) eliminating flash of unstyled content (FOUC).
* Persists user theme choice in `localStorage` with system preference auto-detection (`prefers-color-scheme`).

**Feature (Global Region Coverage)**
* Expanded the region selector from 7 to 40+ BigQuery regions grouped by geography, including full Europe, Asia-Pacific, Middle East, Africa and South America coverage. Regions without organization-scoped INFORMATION_SCHEMA support are annotated rather than hidden.
* Added regional error taxonomy to translate upstream BigQuery exceptions into actionable, context-aware HTTP error messages.
* Added dynamic preservation of custom/exotic regions in the UI select element.

**Feature (Dynamic Regional Pricing)**
* Replaced all hardcoded US-only pricing defaults ($6.25/TiB on-demand, $0.02/GiB storage) with dynamic per-region pricing resolved from an embedded BigQuery Cloud Billing SKU dataset covering 59 regions.
* Added `src/pricing.py` — a zero-dependency leaf module that parses the SKU JSON once at import and provides O(1) region-based price lookups via `get_pricing(region)`.
* Pydantic `model_validator` hooks on `StorageParams`, `JobAnalysisParams`, and `UserProfilerParams` auto-fill regional prices from the user's selected region — no API changes required.
* Report generator (`FindingsSynthesizer`, `HTMLReportRenderer`) now renders regionally-accurate rates in all cost tables, disclaimers, and glossary text.
* Added `GET /api/pricing?region=<region>` endpoint exposing resolved on-demand, storage (logical + physical), and editions rates for any supported region.
* Resolved on-demand and storage pricing dynamically from the embedded BigQuery Cloud Billing SKU dataset covering 59 regions. BigQuery Editions slot pricing defaults to the global baseline ($0.06/slot-hour Enterprise PAYG).

**Feature (Enterprise Cloud Run Deployment Automation)**
* Added split-privilege Cloud Run deployment script (`deploy/deploy_cloud_run.sh`) supporting `--emit-iam` (zero-mutation review for platform/security admins) and `--deploy` (automated service account provisioning and private deployment with IAM authentication).

**Feature (Google Cloud Shell One-Click Deployment)**
* Added Google Cloud Shell as an official zero-setup deployment option with a one-click "Open in Cloud Shell" badge. Runs entirely in the browser with pre-configured Python 3, Git, and the Google Cloud SDK.
* Positioned Cloud Shell as the recommended path for users evaluating on Windows workstations, bypassing corporate PowerShell restrictions, execution policies, PATH issues, and compiler prerequisites.
* Added step-by-step instructions for connecting via Cloud Shell's native Web Preview on port 8080 through Google's secure HTTPS proxy.
* Restructured setup into three tiers: Option 1 (Local Development), Option 2 (Google Cloud Shell), and Option 3 (Cloud Run Serverless Container).

**Fixed**
* Project discovery via `TABLE_STORAGE_BY_ORGANIZATION` now accepts either region convention. Passing a prefixed region such as `region-us` previously produced an invalid `region-region-us` qualifier and returned no projects.

**Change (CI Foundation & Bundle Synchronization)**
* Added automated GitHub Actions CI workflow (`.github/workflows/ci.yml`) covering test suites, coverage, and bundle synchronization.
* Added frontend bundle synchronization gate (`scripts/sync_docs_bundle.sh`, `tests/test_bundle_sync.py`) enforcing SHA-256 byte identity between `static/` and `docs/static/`.
* Added automated region option generator (`scripts/generate_region_options.py`) to sync HTML dropdown options with `src/constants.py`.
* Standardized product naming from "BigQuery FinOps Optimizer" to "FinOps Optimizer for BigQuery" across documentation, PRD, and issue templates.
...
- **AI Model Catalog Alignment:** Updated model pricing and documentation references to Gemini 3.7 Flash and Gemini 3.5 Flash-Lite across interactive calculators, documentation specs, and CI workflows.
...
**Feature**
Updated the AI Doctor module to use `gemini-3.7-flash` as the default model in the dropdown and backend, with `gemini-3.5-flash-lite` as the secondary option for semantic SQL anti-pattern analysis and optimization recommendations.
