...
resource "terraform_data" "iam_scope_guard" {
  lifecycle {
    precondition {
      condition     = var.iam_scope != "organization" || try(length(var.org_id) > 0, false)
      error_message = "org_id is required when iam_scope is \"organization\"."
    }

    precondition {
      condition     = var.iam_scope != "folder" || try(length(var.folder_id) > 0, false)
...
resource "google_organization_iam_member" "runtime" {
  for_each = var.iam_scope == "organization" ? toset(local.scope_sa_roles) : toset([])
