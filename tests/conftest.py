"""
Shared test fixtures for the FinOps Optimizer for BigQuery test suite.
...
def _mock_default_adc(monkeypatch):
    """Ensure tests run completely offline without requiring real GCP Application Default Credentials."""
    import google.auth
...
def single_pass_rows(*rows):
    """Mock `QueryJob.result()` the way BigQuery actually behaves.

    Assign to ``mock_job.result.side_effect``, not ``.return_value``.

    The real return is a `RowIterator`: a ONE-PASS iterator that raises
    ``ValueError("Iterator has already started")`` if anything traverses it a
    second time. A mock returning a plain list is re-iterable, so it hides
    double-consumption bugs completely -- which is how `/api/ai/analyze`
    shipped a guaranteed 500 (a migration-candidate filter consumed the chunk
    rows before the findings loop needed them) against a fully green suite.

    Each call returns a FRESH iterator, because each query gets its own result.
    """
    return lambda *args, **kwargs: iter(rows)
