...
from tests.conftest import single_pass_rows
...
def test_ai_doctor_schema_aware_analysis():
    ...
    with patch("src.main.init_bq_client_and_resolve_project") as mock_init:
        ...
        mock_job.result.side_effect = single_pass_rows(mock_ai_row)
        ...
        assert "endpoint => 'https://aiplatform.googleapis.com/v1/projects/acme-sandbox/locations/global/publishers/google/models/gemini-3.7-flash'" in called_sql
...
def test_ai_doctor_custom_model_selection():
    ...
    with patch("src.main.init_bq_client_and_resolve_project") as mock_init:
        ...
        called_sql = called_args[0]
        assert "gemini-3.7-flash" in called_sql
...
def test_ai_doctor_hybrid_editions_fallback():
    ...
    with patch("src.main.init_bq_client_and_resolve_project") as mock_init:
        ...
        mock_job.result.side_effect = single_pass_rows(mock_editions_row)
        mock_bq_client.query.return_value = mock_job
...
def test_ai_doctor_all_ai_generate_failures_raise_502():
    """Verify that if all AI.GENERATE rows fail with status error, the endpoint raises 502 instead of 200 OK with []."""
    mock_failed_row = MagicMock(
        job_id="job_failed_ai_1",
        user_email="data@acme.com",
        query="SELECT 1",
        worst_job={
            "job_id": "job_failed_ai_1",
        ...
        ai_struct={
            "result": None,
            "status": "INVALID_ARGUMENT: Thinking level is unsupported: THINKING_LEVEL_MINIMAL"
        },
        tables_referenced_count=0,
        tables_found_count=0,
    )
    ...
    with patch("src.main.init_bq_client_and_resolve_project") as mock_init:
        ...
        mock_job.total_bytes_billed = 0
        mock_job.cache_hit = False
        mock_job.job_id = "mock_job_fail"
        mock_job.result.side_effect = single_pass_rows(mock_failed_row)
        mock_bq_client.query.return_value = mock_job
        ...
        assert "AI.GENERATE failed for all 1 analyzed queries" in response.json()["detail"]
        assert "THINKING_LEVEL_MINIMAL" in response.json()["detail"]


def test_ai_doctor_all_clean_queries_return_200_empty():
    """Verify that if all analyzed queries return NO_ANTI_PATTERNS_FOUND, endpoint returns 200 OK with []."""
    mock_clean_row = MagicMock(
        job_id="job_clean_1",
        user_email="data@acme.com",
        query="SELECT 1",
        worst_job={
    ...
    with patch("src.main.init_bq_client_and_resolve_project") as mock_init:
        ...
        mock_job.total_bytes_billed = 0
        mock_job.cache_hit = False
        mock_job.job_id = "mock_job_clean"
        mock_job.result.side_effect = single_pass_rows(mock_clean_row)
        mock_bq_client.query.return_value = mock_job
...
def test_ai_doctor_continues_to_next_chunk_when_first_chunk_clean():
    """Verify that if Chunk 1 (jobs 1-5) returns NO_ANTI_PATTERNS_FOUND, AI Doctor continues to Chunk 2 to fulfill params.limit."""
    def make_discovery_row(idx: int):
        return MagicMock(
            job_id=f"job_{idx}",
            user_email="data@acme.com",
            query=f"SELECT {idx}",
            ...
            execution_count=1,
            annualized_cost_usd=10.0,
            optimization_potential_score=100.0 - idx,
            total_slot_ms=5000,
            total_bytes_billed=1000,
            total_bytes_processed=1000,
    ...
    discovery_rows = [make_discovery_row(i) for i in range(1, 7)]  # 6 candidates (Chunk 1 = 1..5, Chunk 2 = 6)
    ...
    chunk1_rows = [
        MagicMock(
            ...
            tables_referenced_count=0,
            tables_found_count=0,
            ai_struct={"result": "NO_ANTI_PATTERNS_FOUND"},
        )
        for i in range(1, 6)
    ]
    chunk2_rows = [
    ...
    with patch("src.main.init_bq_client_and_resolve_project") as mock_init:
        ...
        call_results = [
            discovery_rows,  # Call 1: discovery_sql
            chunk1_rows,     # Call 2: Chunk 1 AI.GENERATE (all clean)
            chunk2_rows,     # Call 3: Chunk 2 AI.GENERATE (1 finding)
        ]
        ...
        def query_side_effect(sql, *args, **kwargs):
            ...
            job.cache_hit = False
            job.job_id = "mock_job"
            rows = call_results.pop(0) if call_results else []
            job.result.side_effect = single_pass_rows(*rows)
            return job
